...
def addIndent(html_lines, target_title, svg_list):
    ...
    for line in html_lines:
        if '<title>' + target_title + '</title>' in line:                   # If we hit the chart header
            ...
            if line[0] == ' ':
                indent_level = int(indent_chars / 4)
                indent_unit = '    '
                indent = indent_unit * indent_level
            if line[0] == '\t':
                indent_level = indent_chars
                indent_unit = '\t'
                indent = indent_unit * indent_level
        index += 1
...
def createSVGRect(class_str, x, y, width, height, rgb_color, stroke, stroke_width):
    if class_str is None:
        rect_tag = '<rect '
    else:
        rect_tag = '<rect class="' + class_str + '" '
    ...
    if stroke is None:
        rect_tag += 'style="fill:rgb' + str(rgb_color) + '" />'
    else:
        rect_tag += 'style="fill:rgb' + str(rgb_color) + ';'
        rect_tag += 'stroke:' + str(stroke) + ';'
        rect_tag += 'stroke-width:' + str(stroke_width) + '" />'

    return rect_tag


# Creates an SVG line tag with the given parameters
def createSVGLine(class_str, x1, y1, x2, y2, style_dict):
    line_tag = '<line'
    if class_str: line_tag += ' class="' + class_str + '"'
    line_tag += ' x1="' + str(x1) + '"'
    line_tag += ' y1="' + str(y1) + '"'
    line_tag += ' x2="' + str(x2) + '"'
    line_tag += ' y2="' + str(y2) + '"'
    if style_dict:
        line_tag += ' style="'
        if style_dict['rgb_color']: line_tag += 'stroke:rgb' + style_dict['rgb_color'] + ';'
        if style_dict['stroke_width']: line_tag += 'stroke-width:' + style_dict['stroke_width'] + ';'
    line_tag += ' />'
...
def createSVGTextCenter(class_str, x, y, text):
    if class_str is None:
        text_tag = '<text '
    else:
        text_tag = '<text class="' + class_str + '" '
...
def createChartBodyDicts(df):
    ...
    for i in range(len(df)):
        if df['Power'][i] != 'None':
            df_dict.append(dict(Region=df['Region'][i], Start=df['Start Year'][i], Finish=df['End Year'][i], Power=df['Power'][i]))
    return df_dict
...
def createChartBodySvgList(body_df_dict, centers_df_dict, params):
    ...
    for dp in body_df_dict:
        ...
        y = (dp['Start'] + params['bottom_date'] + params['header_length'] * params['year_height'])
        ...
        if curr_power == 'None':
            svg_tag = createSVGRect(None, x, y, params['bar_width'], bar_height, powerColorSwitchRGB(curr_power), None, None)
        else:
            svg_tag = createSVGRect(None, x, y, params['bar_width'], bar_height, powerColorSwitchRGB(curr_power), None, None)
        body_svg_list.append(svg_tag)
...
def insertChartBody(path, html_lines, regions):
...
def insertChartRegions(html_lines, params, regions):
    ...
    for region in regions:
        ...
        svg_tag = createSVGRect('regionbox', x, 0, params['bar_width'], params['header_length'], '(255,255,255)', 'black', 2)
        svg_list.append(svg_tag)
...
def insertChartYears(html_lines, params, regions):
    ...
    x2 = vert_line_level
    y2 = chart_height
    svg_list = [(createSVGLine('yearline', x1, y1, x2, y2, None))]
    while year_level < params['bottom_date'] + params['curr_year']:
        x1 = vert_line_level
        y1 = year_level
        x2 = (len(regions) + 1) * params['bar_width']
        y2 = year_level
        svg_list.append(createSVGLine('yearline', x1, y1, x2, y2, None))
...
if __name__ == "__main__":
    ...
    result_html_lines = insertChartBody(BOT_data_path, base_html_lines, regions_list)       # Insert the chart body svg tags to the correct place
    ...
    result_html_lines = insertChartYears(result_html_lines, chart_params, regions_list)
