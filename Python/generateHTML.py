""" This script takes the data from the main excel file and uses it to draw the chart, and insert it into the
    html template """

import pandas as pd
import svgelements
import helperfunctions as hf
from config import file_paths, chart_params
...
def inverse_region_switch(xlevel):
...
def power_color_switch_rgb(power):
...
def add_indent(html_lines, group_id, svg_list):
...
def create_chart_body_dicts(df):
...
def create_center_regions_dicts(df):
...
def create_chart_body_svg_list(body_df_dict, centers_df_dict, params):
    ...
    for dp in body_df_dict:
        ...
        x = region_switch(dp['Region']) * params['bar_width']
        ...
        box_data_dict = {'data-start-year': hf.make_year_bc(str(dp['Start'])),
                         'data-end-year': hf.make_year_bc(str(dp['End'])),
                         'data-region': curr_region,
                         'data-is-center': 'false'}
        style_dict = {'fill:rgb': power_color_switch_rgb(curr_power)}
        ...
        if iteration > 1:
            if centers_df_dict[dp['Power']]['Center'] == curr_region and iteration == centers_df_dict[dp['Power']]['Iteration']:  # If this box is correct iteration and region to be the center
                ...
                body_svg_list.insert(len(body_svg_list) - 2, svgelements.Rect(None, class_list, None, box_data_dict, style_dict,
                                                                              x, y, params['bar_width'], bar_height, rx, ry).createtag())
            continue
        body_svg_list.append(
            svgelements.OpenGroup(group_name, None, None, None, None, None, ['translate(0 0)'], None).createtag())     # Add an opening group tag
        if centers_df_dict[dp['Power']]['Center'] == curr_region and iteration == centers_df_dict[dp['Power']]['Iteration']:    # If this box is correct iteration and region to be the center
            ...
            if bar_height > (2 * params['body_font_size']):         # If the bar is big enough to display the text
                body_svg_list.append(svgelements.Text(None, ['powertitle'], None, None, None,
                                                      title_x, title_y, 'middle', 'middle', hf.truncate_text(curr_power, 11)).createtag())  # Create the text tag and put it in the group that is added after all of the rectangles
            else:
        ...
        body_svg_list.append(svgelements.Animate(None, ['powerfocusanimationtranslate'], None, None, None,
                                                 'transform', 'XML', 'translate', '0 0', '0 0', '0.75s', 'indefinite', 'forward', 'none').create_tag())
        body_svg_list.append(svgelements.Animate(None, ['powerfocusanimationopacity'], None, None, None,
                                                 'opacity', 'XML', 'translate', '1', '0', '0.75s', 'indefinite', 'forward', 'none').create_tag())
        body_svg_list.append('</g>')
...
def insert_chart_body(path, html_lines):
    ...
    chart_body_df_dict = create_chart_body_dicts(chart_body_df)                                                    # Turn the chart body dataframe into a list of dictionaries
    ...
    center_regions_df_dict = create_center_regions_dicts(center_regions_df)                                        # Turn the center regions dataframe into a list of dictionaries
    chart_body_svg_list = create_chart_body_svg_list(chart_body_df_dict, center_regions_df_dict, chart_params)      # Turn the list of dictionaries into a list of SVG grouped tags

    # Add the proper indentation to every line and insert it into the html lines
    new_html_lines = add_indent(html_lines, 'timelinebody', chart_body_svg_list)
    return new_html_lines


def insert_chart_regions(html_lines, params, regions):
    """ Inserts the x-axis headings for the chart """

    # Create an SVG rect for every region in the chart
    svg_list = []
    for region in regions:
        x = region_switch(region) * params['bar_width']
        y = 0
        ...
        text_data_dict = {'data-xlevel': str(region_switch(region) + 1),
        ...
        svg_list.append(svgelements.Animate(None, ['regionfocusanimationtranslate'], None, None, None,
                                            'transform', 'XML', 'translate', '0 0', '0 0', '0.75s', 'indefinite', 'forward', 'none').create_tag())   # Add the animation for the header groups to move
        svg_list.append(svgelements.Animate(None, ['regionfocusanimationopacity'], None, None, None,
                                            'opacity', 'XML', 'translate', '0 0', '0 0', '0.75s', 'indefinite', 'forward', 'none').create_tag())   # Add the animation for the header groups to fade
        svg_list.append('</g>')

    # Add the proper indentation to every line and insert it into the html lines
    new_html_lines = add_indent(html_lines, 'regions', svg_list)
    return new_html_lines


def insert_chart_years(html_lines, params, regions):
    ...
    # Add the proper indentation to every line and insert it into the html lines
    new_html_lines = add_indent(new_html_lines, 'timelineyearlines', format_svg_list)
    new_html_lines = add_indent(new_html_lines, 'pyearlines', primary_lines_svg_list)
    new_html_lines = add_indent(new_html_lines, 'syearlines', secondary_lines_svg_list)
    new_html_lines = add_indent(new_html_lines, 'pyearlabels', p_year_labels_svg_list)
    new_html_lines = add_indent(new_html_lines, 'syearlabels', s_year_labels_svg_list)
    return new_html_lines
...
if __name__ == "__main__":
    ...
    base_html_lines = hf.read_file(file_paths['base_HTML'])  # Get the HTML template
    result_html_lines = insert_chart_body(file_paths['BOT_data'], base_html_lines, )  # Insert the chart body svg tags to the correct place
    result_html_lines = insert_chart_regions(result_html_lines, chart_params, regions_list)  # Insert the chart x-axis header svg tags to the correct place
    result_html_lines = insert_chart_years(result_html_lines, chart_params, regions_list)  # Insert the chart y-axis tags and the gridlines
    hf.write_file(file_paths['final_HTML'], result_html_lines)
